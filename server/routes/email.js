...
router.get('/all', (req, res, next) => {
    ...
    function(callback) {
      req.app.get("db").collection('users').findOne(details, (err, user) => {
        ...
        if (err || !user) {
          console.log(err, 1);
          callback({error: 'Error during user finding.'});
        }
        callback(null, user);
      });
    },
    function(user, callback) {
      if (user.members) {
        ...
        {  _id: { $in: user.members }}, {fields:{password: 0, members:0}}).toArray((err, members) => {
          ...
          if (err) {
            console.log(err, 2);
            callback({error: 'Error during members finding.'});
          }
          console.log(members);
    ...
    function(callback) {
      req.app.get("db").collection('employees').find({ heads: id }, {fields:{password: 0, heads : 0}}).toArray((err, employees) => {
        console.log(err, employees);
          if (err) {
            console.log(err, 3);
            callback({error: 'Error during employees finding.'});
          }
          if (employees) {
        ...
        req.app.get("db").collection('users').findOne(details, (err3, result3) => {
          ...
          req.app.get("db").collection('users').findOneAndUpdate({ _id: result3._id }, {
            $set: { members: members }
              ...
              const allMembers = (result4.value.members) ? result4.value.members : [];
